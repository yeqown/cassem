...
func (f *retentionCompactFakeCoord) getKV(key string) (*apicassemdb.Entity, error) {
...
func (f *retentionCompactFakeCoord) iterate(param *rangeParam) (*apicassemdb.RangeResp, error) {
...
func addMetadata(t *testing.T, fake *retentionCompactFakeCoord, root string, md *concept.ElementMetadata) {
	...
	fake.entities[key] = apicassemdb.NewEntityWithCreated(key, mustProtoBytes(t, md), 0, time.Now().Unix())
}
...
func addVersion(t *testing.T, fake *retentionCompactFakeCoord, root string, version int32, createdAt time.Time) string {
	...
	fake.entities[key] = apicassemdb.NewEntityWithCreated(key, mustProtoBytes(t, &concept.Element{Version: version, Raw: []byte("v")}), 0, createdAt.Unix())
	return key
}
...
func addOperation(t *testing.T, fake *retentionCompactFakeCoord, appID, env, key string, operatedAt time.Time) string {
	...
	fake.entities[opKey] = apicassemdb.NewEntityWithCreated(opKey, mustProtoBytes(t, op), 0, operatedAt.Unix())
	return opKey
}
...
func TestCompactElementHistoryKeepsProtectedVersions(t *testing.T) {
	...
	resp, err := compactElementHistory(fake, &apicassemdb.CompactElementHistoryReq{
		ElementKey:           root,
		KeepVersionCount:     2,
...
func TestCompactElementHistoryReportsMalformedVersionKeys(t *testing.T) {
	...
	fake.entities[malformedVersionKey] = apicassemdb.NewEntityWithCreated(malformedVersionKey, []byte("bad"), 0, now.Add(-60*24*time.Hour).Unix())
...
func TestCompactElementHistoryReturnsPartialFailures(t *testing.T) {
	...
	t.Run("corrupted operation payload", func(t *testing.T) {
		...
		fake.entities[corruptedOperation] = apicassemdb.NewEntityWithCreated(corruptedOperation, []byte("bad"), 0, now.Add(-200*24*time.Hour).Unix())
...
func TestCompactElementHistoryReturnsErrors(t *testing.T) {
	...
	t.Run("nil coordinator", func(t *testing.T) {
		_, err := compactElementHistory(nil, &apicassemdb.CompactElementHistoryReq{ElementKey: root}, now)
		require.Error(t, err)
	})

	t.Run("empty element key", func(t *testing.T) {
		_, err := compactElementHistory(newRetentionCompactFakeCoord(), &apicassemdb.CompactElementHistoryReq{}, now)
		require.Error(t, err)
	})

	t.Run("missing metadata", func(t *testing.T) {
		_, err := compactElementHistory(newRetentionCompactFakeCoord(), &apicassemdb.CompactElementHistoryReq{
			ElementKey:           root,
			KeepVersionCount:     1,
			KeepVersionSeconds:   int64((30 * 24 * time.Hour).Seconds()),
			KeepOperationSeconds: int64((180 * 24 * time.Hour).Seconds()),
	...
	t.Run("invalid metadata", func(t *testing.T) {
		fake := newRetentionCompactFakeCoord()
		fake.entities[concept.WithMetadataSuffix(root)] = apicassemdb.NewEntityWithCreated(concept.WithMetadataSuffix(root), []byte("bad"), 0, now.Unix())
		_, err := compactElementHistory(fake, &apicassemdb.CompactElementHistoryReq{
			ElementKey:           root,
			KeepVersionCount:     1,
			KeepVersionSeconds:   int64((30 * 24 * time.Hour).Seconds()),
			KeepOperationSeconds: int64((180 * 24 * time.Hour).Seconds()),
