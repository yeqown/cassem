...
func (d *app) deleteClusterMember(nodeID uint64) error {
	return d.raft.UnsetKV(&apicassemdb.UnsetKVReq{Key: clusterMemberKey(nodeID)})
}

func (d *app) getClusterMember(nodeID uint64) (clusterMemberRecord, error) {
	entity, err := d.raft.GetKV(&apicassemdb.GetKVReq{Key: clusterMemberKey(nodeID)})
	if err != nil {
...
func (d *app) listMembers() ([]*apicassemdb.ClusterMember, error) {
	resp, err := d.raft.Range(&apicassemdb.RangeReq{Key: clusterMemberKeyPrefix, Limit: 100})
	if err != nil {
	...
	members := make([]*apicassemdb.ClusterMember, 0, len(resp.GetEntities()))
	for _, entity := range resp.GetEntities() {
		...
		members = append(members, &apicassemdb.ClusterMember{
